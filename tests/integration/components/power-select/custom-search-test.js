...
module(
  ...
  function (hooks) {
    ...
    test('When you pass a custom search action instead of options, opening the select show a "Type to search" message in a list element', async function (assert) {
      ...
      await render(hbs`
      <PowerSelect @search={{this.searchFn}} @onChange={{action (mut this.foo)}} as |number|>
        {{number}}
      </PowerSelect>
    `);
    ...
    test("The search text shouldn't appear if options are loading", async function (assert) {
      ...
      await render(hbs`
      <PowerSelect @options={{this.options}} @search={{this.searchFn}} @onChange={{action (mut this.foo)}} @searchEnabled={{true}} as |number|>
        {{number}}
      </PowerSelect>
    `);
    ...
    test('The "type to search" message can be customized passing `@searchMessage=something`', async function (assert) {
      ...
      this.searchFn = function () {};
      await render(hbs`
      <PowerSelect @search={{this.searchFn}} @searchMessage="Type the name of the thing" @onChange={{action (mut this.foo)}} as |number|>
        {{number}}
      </PowerSelect>
    `);
    ...
    test('When the search resolves to an empty array then the custom "No results" message appears', async function (assert) {
      ...
      await render(hbs`
      <PowerSelect @search={{this.searchFn}} @noMatchesMessage="Meec. Try again" @onChange={{action (mut this.foo)}} @searchEnabled={{true}} as |number|>
        {{number}}
      </PowerSelect>
    `);
    ...
    test('On an select with a selected value, if after a search this value is not among the options the first element is highlighted', async function (assert) {
      ...
      await render(hbs`
      <PowerSelect @search={{this.searchFn}} @options={{this.numbers}} @selected={{this.selected}} @onChange={{action (mut this.foo)}} @searchEnabled={{true}} as |number|>
        {{number}}
      </PowerSelect>
    `);
    ...
    test('When received both options and search, those options are shown when the dropdown opens before the first search is performed', async function (assert) {
      ...
      await render(hbs`
      <div id="different-node"></div>
      <PowerSelect @options={{this.numbers}} @search={{this.searchFn}} @onChange={{action (mut this.foo)}} @searchEnabled={{true}} as |number|>
        {{number}}
      </PowerSelect>
    `);
    ...
    test("Don't return from the search action and update the options instead also works as an strategy", async function (assert) {
      ...
      await render(hbs`
      <div id="different-node"></div>
      <PowerSelect @options={{this.selectedOptions}} @search={{this.searchFn}} @onChange={{action (mut this.foo)}} @searchEnabled={{true}} as |number|>
        {{number}}
      </PowerSelect>
    `);
    ...
    //   this.render(hbs`
    //     <div id="different-node"></div>
    //     <PowerSelect @options={{this.selectedOptions}} @search={{this.searchFn}} @onChange={{action (mut this.foo)}} as |number|>
    //       {{number}}
    //     </PowerSelect>
    //   `);
    ...
    test('The lastSearchedText of the yielded publicAPI in multiple selects is updated only when the async search for it finishes', async function (assert) {
      ...
      await render(hbs`
      <PowerSelectMultiple @options={{this.numbers}} @search={{this.searchFn}} @onChange={{action (mut this.foo)}} @searchEnabled={{true}} as |number select|>
        {{number}}:{{select.lastSearchedText}}
      </PowerSelectMultiple>
    `);
    ...
    test('BUGFIX: Destroy a component why an async search is pending does not cause an error', async function (assert) {
      ...
      await render(hbs`
      {{#if this.visible}}
        <PowerSelectMultiple @options={{this.numbers}} @search={{this.searchFn}} @onChange={{action (mut this.foo)}} @searchEnabled={{true}} as |number searchTerm|>
          {{number}}:{{searchTerm}}
        </PowerSelectMultiple>
      {{/if}}
    `);
    ...
    test('BUGFIX: When the given options are a promise and a search function is provided, clearing the search must display the results of the original promise', async function (assert) {
      ...
      await render(hbs`
     <PowerSelect @options={{this.numbersPromise}} @search={{this.searchFn}} selected=foo @onChange={{action (mut this.foo)}} @searchEnabled={{true}} as |number|>
       {{number}}
     </PowerSelect>
    `);
    ...
    test('BUGFIX: If the user provides a custom matcher, that matcher receives the entire option even if the user also provided a searchField', async function (assert) {
      ...
      await render(hbs`
      <PowerSelectMultiple @options={{this.countries}} @matcher={{this.matcherFn}} @searchField="name" @onChange={{action (mut this.foo)}} @searchEnabled={{true}} as |number searchTerm|>
        {{country.name}}
      </PowerSelectMultiple>
    `);
    ...
    test('If the value returned from an async search is cancellable and before it completes a new search is fired, the first value gets cancelled', async function (assert) {
      ...
      await render(hbs`
      <PowerSelect @search={{perform this.obj.searchTask}} @onChange={{action (mut this.foo)}} @searchEnabled={{true}} as |number|>
        {{number}}
      </PowerSelect>
    `);
    ...
    test('If the value returned from an async search of a multiple-select is cancellable and before it completes a new search is fired, the first value gets cancelled', async function (assert) {
      ...
      await render(hbs`
      <PowerSelectMultiple @search={{perform this.obj.searchTask}} @onChange={{action (mut this.foo)}} @searchEnabled={{true}} as |number|>
        {{number}}
      </PowerSelectMultiple>
    `);
