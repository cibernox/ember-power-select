...
test('the cursor position of the searchbox of single selects is preserved when the search is invoked from the outside', function(assert) {
  assert.expect(2);
  this.numbers = numbers;
  this.handleKeydown = (select) => {
    select.actions.search('abcXdef');
  };
  this.render(hbs`
    {{#power-select options=numbers onkeydown=(action handleKeydown) selected=foo onchange=(action (mut foo)) as |option|}}
      {{option}}
    {{/power-select}}
  `);
  ...
  typeInSearch('abcdef');
  input.selectionStart = 3; // between the `c` and the `d`;
  assert.equal(input.selectionStart, 3, 'The cursor is between the c and the d');
  triggerKeydown(input, 17); // Ctrl key. It's a dead key.
  assert.equal(input.selectionStart, 3, 'The cursor position has been preserved');
});

test('the cursor position of the searchbox of multiple selects is preserved when the search is invoked from the outside', function(assert) {
  assert.expect(2);
  this.numbers = numbers;
  this.handleKeydown = (select) => {
    select.actions.search('abcXdef');
  ...
  this.render(hbs`
    {{#power-select-multiple options=numbers onkeydown=(action handleKeydown) selected=foo onchange=(action (mut foo)) as |option|}}
      {{option}}
    {{/power-select-multiple}}
  `);
