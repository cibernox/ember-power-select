...
module('Integration | Component | Ember Power Select (Public actions)', function(hooks) {
  ...
  test('The search action of multiple selects action receives the search term and the public API', async function(assert) {
    ...
    await render(hbs`
      <PowerSelectMultiple @options={{this.numbers}} @selected={{this.foo}} @onChange={{action (mut this.foo)}} @search={{handleSearch}} @searchEnabled={{true}} as |number|>
        {{number}}
      </PowerSelectMultiple>
    `);
  ...
  test('The onchange of single selects action receives the selection and the public API', async function(assert) {
    ...
    await render(hbs`
      <PowerSelect @options={{this.numbers}} @selected={{this.foo}} @onChange={{handleChange}} as |number|>
        {{number}}
      </PowerSelect>
    `);
  ...
  test('The onchange of multiple selects action receives the selection and the public API', async function(assert) {
    ...
    await render(hbs`
      <PowerSelectMultiple @options={{this.numbers}} @selected={{this.foo}} @onChange={{handleChange}} as |number|>
        {{number}}
      </PowerSelectMultiple>
    `);
  ...
  test('The onKeydown of single selects action receives the public API and the keydown event when fired on the searchbox', async function(assert) {
    ...
    await render(hbs`
      <PowerSelect @options={{this.numbers}} @selected={{this.foo}} @onKeydown={{onKeyDown}} @onChange={{action (mut this.foo)}} @searchEnabled={{true}} as |number|>
        {{number}}
      </PowerSelect>
    `);
  ...
  test('The onKeydown of multiple selects action receives the public API and the keydown event', async function(assert) {
    ...
    await render(hbs`
      <PowerSelectMultiple @options={{this.numbers}} @selected={{this.foo}} @onKeydown={{onKeyDown}} @onChange={{action (mut this.foo)}} @searchEnabled={{true}} as |number|>
        {{number}}
      </PowerSelectMultiple>
    `);
  ...
  test('The onBlur of single selects action receives the public API and the event', async function(assert) {
    ...
    await render(hbs`
      <PowerSelect @options={{this.numbers}} @selected={{this.foo}} @onBlur={{handleBlur}} @onChange={{action (mut this.foo)}} as |number|>
        {{number}}
      </PowerSelect>
  ...
  test('The onBlur of multiple selects action receives the public API and the focus event', async function(assert) {
    ...
    await render(hbs`
      <PowerSelectMultiple @options={{this.numbers}} @selected={{this.foo}} @onBlur={{handleBlur}} @onChange={{action (mut this.foo)}} @searchEnabled={{true}} as |number|>
        {{number}}
      </PowerSelectMultiple>
      <input type="text" id="other-element"/>
    `);
  ...
  test('the `@onOpen` action is invoked just before the dropdown opens', async function(assert) {
    ...
    await render(hbs`
      <PowerSelect @options={{this.numbers}} @onChange={{action (mut this.foo)}} @onOpen={{handleOpen}} as |option|>
        {{option}}
      </PowerSelect>
    `);
  ...
  test('returning false from the `@onOpen` action prevents the multiple select from opening', async function(assert) {
    ...
    await render(hbs`
      <PowerSelectMultiple @options={{this.numbers}} @onChange={{action (mut this.foo)}} @onOpen={{handleOpen}} as |option|>
        {{option}}
      </PowerSelectMultiple>
    `);
  ...
  test('the `@onClose` action is invoked just before the dropdown closes', async function(assert) {
    ...
    await render(hbs`
      <PowerSelect @options={{this.numbers}} @onChange={{action (mut this.foo)}} @onClose={{handleClose}} as |option|>
        {{option}}
      </PowerSelect>
    `);
  ...
  test('returning false from the `@onClose` action prevents the multiple select from closing', async function(assert) {
    ...
    await render(hbs`
      <PowerSelectMultiple @options={{this.numbers}} @onChange={{action (mut this.foo)}} @onClose={{handleClose}} as |option|>
        {{option}}
      </PowerSelectMultiple>
    `);
  ...
  test('the `oninput` action is invoked when the user modifies the text of the search input on single selects, and the search happens', async function(assert) {
    ...
    await render(hbs`
      <PowerSelect @options={{this.numbers}} @selected={{this.foo}} @onInput={{handleInput}} @onChange={{action (mut this.foo)}} @searchEnabled={{true}} as |number|>
        {{number}}
      </PowerSelect>
    `);
  ...
  test('the `oninput` action is invoked when the user modifies the text of the search input on multiple selects, and the search happens', async function(assert) {
    ...
    await render(hbs`
      <PowerSelectMultiple @options={{this.numbers}} @selected={{this.foo}} @onInput={{handleInput}} @onChange={{action (mut this.foo)}} @searchEnabled={{true}} as |number|>
        {{number}}
      </PowerSelectMultiple>
    `);
  ...
  test('the `highlight` action of the public api passed to the public actions works as expected', async function(assert) {
    ...
    await render(hbs`
      <PowerSelect @options={{options}} @onChange={{action (mut this.foo)}} @onOpen={{handleOpen}} as |option|>
        {{option}}
      </PowerSelect>
    `);
    await clickTrigger();
  ...
  test('The programmer can use the received public API to perform searches in single selects', async function(assert) {
    ...
    await render(hbs`
      <PowerSelect @options={{this.numbers}} @selected={{this.foo}} @onOpen={{initSearch}} @onChange={{action (mut this.foo)}} @searchEnabled={{true}} as |number|>
        {{number}}
      </PowerSelect>
    `);
  ...
  test('The programmer can use the received public API to perform searches in mutiple selects', async function(assert) {
    ...
    await render(hbs`
      <PowerSelectMultiple @options={{this.numbers}} @selected={{this.foo}} @onOpen={{initSearch}} @onChange={{action (mut this.foo)}} @searchEnabled={{true}} as |number|>
        {{number}}
      </PowerSelectMultiple>
    `);
  ...
  test('The single component invokes the `registerAPI` action with the public API object', async function(assert) {
    ...
    await render(hbs`
      <PowerSelect @options={{this.numbers}} @selected={{this.foo}} @onChange={{action (mut this.foo)}} @registerAPI={{storeAPI}} as |number|>
        {{number}}
      </PowerSelect>
    `);
  });
  ...
  test('The multiple component invokes the `registerAPI` action with the public API object', async function(assert) {
    ...
    await render(hbs`
      <PowerSelectMultiple @options={{this.numbers}} @selected={{this.foo}} @onChange={{action (mut this.foo)}} @registerAPI={{storeAPI}} as |number|>
        {{number}}
      </PowerSelectMultiple>
    `);
  });
  ...
  test('The given `scrollTo` function is invoked when a single select wants to scroll to an element', async function(assert) {
    ...
    await render(hbs`
      <PowerSelect @options={{this.numbers}} @selected={{this.foo}} @onChange={{action (mut this.foo)}} @registerAPI={{this.storeAPI}} @scrollTo={{scrollTo}} as |number|>
        {{number}}
      </PowerSelect>
    `);
  ...
  test('The given `scrollTo` function is invoked when a multiple select wants to scroll to an element', async function(assert) {
    ...
    await render(hbs`
      <PowerSelectMultiple @options={{this.numbers}} @selected={{this.foo}} @onChange={{action (mut this.foo)}} @registerAPI={{storeAPI}} @scrollTo={{scrollTo}} as |number|>
        {{number}}
      </PowerSelectMultiple>
    `);
