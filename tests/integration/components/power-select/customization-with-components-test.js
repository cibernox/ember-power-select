  ...
  function(hooks) {
    ...
    test("the `triggerComponent` receives the `onFocus` action that triggers the `onfocus` action in the outside", async function(assert) {
      ...
      this.didFocusInside = function(select, event) {
        ...
        assert.equal(
          typeof select.highlighted,
          "object",
          "select.highlighted is a string"
        );
        assert.equal(
    ...
    test("groupComponent has extension points", async function(assert) {
      ...
      assert.ok(
        PowerSelectGroupComponent,
        "component:custom-group-component must be defined"
      );
    ...
    `);
      ...
      assert
        ...
        .exists(
          { count: 1 },
          "The custom selectedItemComponent renders with the extra.coolFlagIcon customization option triggering some state change."
        );
    });
  }
);
