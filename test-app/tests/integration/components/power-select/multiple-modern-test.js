...
module(
  ...
  function (hooks) {
    ...
    test("Multiple selects don't have a search box by default", async function (assert) {
      ...
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.numbers}} @selected={{this.foo}} @onChange={{fn (mut this.foo)}} as |option|>
        {{option}}
      </PowerSelect>
    `);
    ...
    test('Multiple selects have a search box in the trigger when the search is enabled', async function (assert) {
      ...
      this.numbers = numbers;
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.numbers}} @selected={{this.foo}} @onChange={{fn (mut this.foo)}} @searchEnabled={{true}} as |option|>
        {{option}}
      </PowerSelect>
    `);
    ...
    test('Multiple selects have a search box in the dropdown when the search is enabled and search position is `after-options`', async function (assert) {
      ...
      this.numbers = numbers;
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.numbers}} @selected={{this.foo}} @onChange={{fn (mut this.foo)}} @searchEnabled={{true}} @searchFieldPosition="before-options" as |option|>
        {{option}}
      </PowerSelect>
    `);
      await clickTrigger();
    ...
    test('Selecting an element triggers the onchange action with the list of selected options', async function (assert) {
      ...
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.numbers}} @onChange={{this.change}} as |option|>
        {{option}}
      </PowerSelect>
    `);
    ...
    test('Click an option when there is already another selects both, and triggers the onchange action with them', async function (assert) {
      ...
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.numbers}} @selected={{this.selectedNumbers}} @onChange={{this.change}} as |option|>
        {{option}}
      </PowerSelect>
    `);
    ...
    test('If there is many selections, all those options are styled as `selected`', async function (assert) {
      ...
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.numbers}} @selected={{this.selectedNumbers}} @onChange={{fn (mut this.selectedNumbers)}} as |option|>
        {{option}}
      </PowerSelect>
    `);
    ...
    test('The default filtering works in multiple mode', async function (assert) {
      ...
      this.numbers = numbers;
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.numbers}} @onChange={{fn (mut this.foo)}} @searchEnabled={{true}} as |option|>
        {{option}}
      </PowerSelect>
    `);
    ...
    test('The filtering specifying a searchkey works in multiple model', async function (assert) {
      ...
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.people}} @searchField="name" @onChange={{fn (mut this.foo)}} @searchEnabled={{true}} as |person|>
    ...
    test('The filtering specifying a custom matcher works in multiple model', async function (assert) {
      ...
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.numbers}} @matcher={{this.endsWithMatcher}} @onChange={{fn (mut this.foo)}} @searchEnabled={{true}} as |option|>
        {{option}}
      </PowerSelect>
    `);
    ...
    test('The search using a custom action works int multiple mode', async function (assert) {
      ...
      await render(hbs`
      <PowerSelect @multiple={{true}} @search={{this.searchFn}} @onChange={{fn (mut this.foo)}} @searchEnabled={{true}} as |number|>
        {{number}}
      </PowerSelect>
    `);
    ...
    test('Pressing ENTER over a highlighted element selects it', async function (assert) {
      ...
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.numbers}} @selected={{this.foo}} @onChange={{this.change}} @searchEnabled={{true}} as |option|>
        {{option}}
      </PowerSelect>
    `);
    ...
    test('Pressing ENTER over a highlighted element on a select without a searchbox selects it', async function (assert) {
      ...
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.numbers}} @selected={{this.foo}} @onChange={{this.change}} as |option|>
        {{option}}
      </PowerSelect>
    `);
    ...
    test('Pressing ENTER over a highlighted element what is already selected closes the select without doing anything and focuses the trigger', async function (assert) {
      ...
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.numbers}} @selected={{this.selected}} @onChange={{this.didChange}} @searchEnabled={{true}} as |option|>
        {{option}}
      </PowerSelect>
    `);
    ...
    test("Pressing BACKSPACE on the search input when it's empty removes the last selection and performs a search for that text immediately, opening the select if closed", async function (assert) {
      ...
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.numbers}} @onChange={{this.didChange}} @selected={{this.selected}} @searchEnabled={{true}} as |option|>
        {{option}}
      </PowerSelect>
    `);
    ...
    test("Pressing BACKSPACE on the search input when it's empty removes the last selection and performs a search for that text immediatly (when options are not strings)", async function (assert) {
      ...
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.countries}} @selected={{this.country}} @onChange={{this.didChange}} @searchField="name" @searchEnabled={{true}} as |c|>
        {{c.name}}
      </PowerSelect>
    `);
      await clickTrigger();
    ...
    test('The placeholder is only visible when no options are selected', async function (assert) {
      ...
      this.numbers = numbers;
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.numbers}} @selected={{this.foo}} @onChange={{fn (mut this.foo)}} @placeholder="Select stuff here" @searchEnabled={{true}} as |option|>
        {{option}}
      </PowerSelect>
    `);
    ...
    test('The placeholder is visible when no options are selected and search is disabled', async function (assert) {
      ...
      this.numbers = numbers;
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.numbers}} @selected={{this.foo}} @onChange={{fn (mut this.foo)}} @placeholder="Select stuff here" as |option|>
        {{option}}
      </PowerSelect>
    `);
    ...
    test('Typing in the input opens the component and filters the options also with async searches', async function (assert) {
      ...
      await render(hbs`
      <PowerSelect @multiple={{true}} @selected={{this.foo}} @onChange={{fn (mut this.foo)}} @search={{this.search}} @searchEnabled={{true}} as |option|>
        {{option}}
      </PowerSelect>
    `);
    ...
    test('The publicAPI is yielded as second argument in multiple selects', async function (assert) {
      ...
      this.numbers = numbers;
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.numbers}} @selected={{this.foo}} @onChange={{fn (mut this.foo)}} @searchEnabled={{true}} as |option select|>
    ...
    test('When hitting ENTER after a search with no results, the component is closed but the onchange function is not invoked', async function (assert) {
      ...
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.numbers}} @selected={{this.foo}} @onChange={{this.handleChange}} @searchEnabled={{true}} as |option|>
        {{option}}
      </PowerSelect>
    `);
    ...
    test('The trigger of multiple selects have a special class to distinguish them from regular ones, even if you pass them another one', async function (assert) {
      ...
      this.numbers = numbers;
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.numbers}} @triggerClass="foobar-trigger" @selected={{this.foo}} @onChange={{fn (mut this.foo)}} as |option|>
        {{option}}
      </PowerSelect>
    `);
    ...
    test('The component works when the array of selected elements is mutated in place instead of replaced', async function (assert) {
      ...
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.numbers}} @selected={{this.selected}} @onChange={{fn (mut this.selected)}} as |option|>
        {{option}}
      </PowerSelect>
    `);
      await clickTrigger();
    ...
    test('When the input inside the multiple select gets focused, the trigger and the dropdown gain special `--active` classes', async function (assert) {
      ...
      this.numbers = numbers;
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.numbers}} @onChange={{fn (mut this.foo)}} as |option|>
        {{option}}
      </PowerSelect>
    `);
    ...
    test('When the power select multiple uses the default component and the search is enabled, and the component receives an specific tabindex, the trigger has tabindex=-1, and the tabindex is applied to the searchbox inside', async function (assert) {
      ...
      this.numbers = numbers;
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.numbers}} @selected={{this.foo}} @onChange={{fn (mut this.foo)}} @tabindex="3" @searchEnabled={{true}} as |option|>
        {{option}}
      </PowerSelect>
    `);
    ...
    test('Multiple selects honor the `defaultHighlighted` option', async function (assert) {
      ...
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.numbers}} @selected={{this.foo}} @onChange={{fn (mut this.foo)}} @defaultHighlighted={{this.defaultHighlighted}} as |option|>
        {{option}}
      </PowerSelect>
    `);
    ...
    test('When there is an option which is disabled the css class "ember-power-select-multiple-option--disabled" should be added', async function (assert) {
      ...
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.countriesWithDisabled}} @selected={{this.foo}} @onChange={{fn (mut this.foo)}} as |option|>
        {{option}}
      </PowerSelect>
    `);
      assert
    ...
    test('The title is rendered in the trigger of multiple selects', async function (assert) {
      ...
      this.numbers = numbers;
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.numbers}} @onChange={{fn (mut this.foo)}} @title="The title" as |option|>
        {{option}}
      </PowerSelect>
    `);
    ...
    test('Multiple selects: The label was rendered when it was passed with `@labelText="Label for select` and is matching with trigger id', async function (assert) {
      ...
      this.numbers = numbers;
      await render(hbs`
      <PowerSelect @multiple={{true}} @options={{this.numbers}} @labelText="Label for select" @onChange={{fn (mut this.foo)}} as |option|>
        {{option}}
      </PowerSelect>
    `);
