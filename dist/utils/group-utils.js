...
function countOptions(collection) {
  ...
  (function walk(collection) {
    ...
    for (let i = 0; i < collection.length; i++) {
      const entry = collection && 'objectAt' in collection && typeof collection.objectAt === 'function' ? collection.objectAt(i) : collection[i];
      if (isGroup(entry)) {
        walk(entry.options);
      } else {
        counter++;
...
function optionAtIndex(originalCollection, index) {
  ...
  return function walk(collection, ancestorIsDisabled) {
    ...
    while (counter <= index && localCounter < length) {
      const entry = collection && 'objectAt' in collection && typeof collection.objectAt === 'function' ? collection.objectAt(localCounter) : collection[localCounter];
      if (isGroup(entry)) {
...
function findOptionWithOffset(options, text, matcher, offset, skipDisabled = false) {
  ...
  (function walk(options, ancestorIsDisabled) {
    ...
    for (let i = 0; i < length; i++) {
      const entry = options && 'objectAt' in options && typeof options.objectAt === 'function' ? options.objectAt(i) : options[i];
