...
class PowerSelectComponent extends Component {
  ...
  handleKeydown(e) {
    ...
    if (this.args.multiple && e.keyCode === 13 && this.storedAPI.isOpen) {
      ...
      if (this.storedAPI.highlighted !== undefined) {
        if (!this.storedAPI.selected || this.storedAPI.selected.indexOf(this.storedAPI.highlighted) === -1) {
  ...
  _defaultMultipleBuildSelection(option, select) {
    if (!this.args.multiple) {
  ...
  focusInput(select) {
    if (select) {
      ...
      const input = root.querySelector(`#ember-power-select-trigger-input-${select.uniqueId}`);
      if (input) {
        input.focus();
      }
    }
