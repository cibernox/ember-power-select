...
QUnit.test('it renders', function(assert) {
  ...
  return this.visit('/fastboot-testing').then(function(data) {
    ...
    assert.strictEqual(statusCode, 200, 'Request is successful');
    assert.strictEqual(headers["content-type"], "text/html; charset=utf-8", 'Content type is correct');
    var select;
    ...
    assert.strictEqual(select.textContent.trim(), '', 'Nothing is selected');

    // Single select with selected
    select = document.querySelector('#single-select-with-selected .ember-power-select');
    assert.ok(select, 'The select has been rendered');
    assert.ok(select.querySelector('.ember-power-select-trigger'), 'It contains a trigger');
    assert.strictEqual(select.textContent.trim(), 'four', 'The forth option is selected');
    ...
    assert.ok(select, 'The select has been rendered');
    assert.ok(select.querySelector('.ember-power-select-trigger'), 'It contains a trigger');
    assert.strictEqual(select.querySelectorAll('.ember-power-select-multiple-option').length, 2, 'There is 2 selected options');
